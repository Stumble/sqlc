...
func (q *Queries) Dump(ctx context.Context, beforeDump ...BeforeDump) ([]byte, error) {
	sql := "SELECT id,name,description,category,price,thumbnail,qrcode,metadata,createdat,updatedat,totalvolume,last30dvolume,floorprice FROM v_items ORDER BY id,name,description,category,price,thumbnail,qrcode,metadata,createdat,updatedat,totalvolume,last30dvolume,floorprice ASC;"
...
func (q *Queries) Load(ctx context.Context, data []byte) error {
	...
	for _, row := range rows {
		_, err := q.db.WExec(ctx, "Load", sql, row.ID, row.Name, row.Description, row.Category, row.Price, row.Thumbnail, row.Qrcode, row.Metadata, row.CreatedAt, row.UpdatedAt, row.Totalvolume, row.Last30dvolume, row.Floorprice)
		if err != nil {
			return err
		}
	}
